...
3. **可插拔 Provider**：`logback-jmx`（现逻辑，支持 reload）与 `boot-actuator-jmx`（兜底，不支持 reload），`auto` 自动探测。
4. **健壮性与打磨**：统一错误退出码与 `--verbose`、支持重置为继承级别、`--object-name` 多 LoggerContext 选择、`--json` 输出、单元测试与目标侧配置文档。
...
flowchart LR
    ...
    PF --> P2["BootActuatorJmxProvider<br/>type=Endpoint,name=Loggers<br/>MBeanInfo 驱动入参"]
...
1. **Provider 优先顺序**：`auto` 下先探测 `ch.qos.logback.classic:Type=...JMXConfigurator,*`（能力最全，含 reload），找不到再探测 `org.springframework.boot:type=Endpoint,name=Loggers`。
2. **禁止写死 Actuator 操作签名**：Boot 文档只说明 JMX 侧入参会做类型转换、`configureLogLevel(name, configuredLevel)` 第二参可能是 `String` 也可能是 `org.springframework.boot.logging.LogLevel` 枚举，尚未实证。实现必须运行时读 `mbsc.getMBeanInfo(name)` 的 `MBeanOperationInfo` 动态构造 `params/signature`：`String` 直接传；自定义类型走 `Class.forName + Enum.valueOf` 构造，失败则明确报错。`doctor` 子命令负责把这层不确定性变成可观测事实。
...
- 走 Actuator 兜底（Boot 2.7）：仅需 classpath 有 `spring-boot-starter-actuator`，无需 `management.*` 配置（默认 JMX include=`*`、loggers 默认暴露）；若目标是 Boot 3.x 才需要显式 `management.endpoints.jmx.exposure.include=health,loggers`。
...
└── src/main/java/com/jmxlogger/
    ...
    │   ├── BootActuatorJmxProvider.java             # [NEW] 解析 loggers()/loggerLevels(name)/configureLogLevel(...)，入参由 MBeanInfo 决定
...
- **性能**：现有 `getLoggerList()` 后对每个 logger 做 2 次 RMI 调用是主要耗时来源。Actuator `loggers()` 是一次拿全量；Logback Provider 侧无法批量，用 `--no-effective` 提供减半往返的逃生口（默认行为不变）。
...
| P3 | Actuator 兜底 | `BootActuatorJmxProvider` + `ProviderFactory` auto | 目标应用未配 `<jmxConfigurator/>` 但带 actuator 时，`--target auto` 自动走 Endpoint Loggers；`reload` 给出替代方案而非崩溃 |
| P4 | 打磨 | `--json`、`set inherit`、`--object-name`、超时控制、单测、Justfile、README | 单元测通过；`get --json` 可被脚本消费 |
| P5 | 未来 Boot 3 预留 | 基于已有 Provider 接口扩展 HTTP Provider / 自定义 endpoint 指引 | 文档化差异（JMX 默认 `health`、logback 无 JMXConfigurator），不写代码实现 |
...
- P3：Actuator 操作签名与返回值形态存在版本差异（`String` vs `LogLevel`、返回值可能是 OpenData 复合结构），因此本阶段以 `doctor` 实测签名驱动，先探测后编码，禁止猜测。
- P4：`--json` 输出一旦发布即成为契约，字段名需一次定稿。
- P5：Boot 1.5 的 actuator 端点属于旧 JMX 命名模型，明确不在支持范围，避免投入。